...
SELECT * FROM MEMBERS;
	...
	-- 전체 회원 출력(SELECT ALL)
	SELECT M_ID, M_NAME, M_EMAIL1, M_EMAIL2, M_NUM, M_REGDATE FROM MEMBERS
	
	-- 회원 하나 출력(SELECT ONE)
	SELECT M_ID, M_NAME, M_EMAIL1, M_EMAIL2, M_NUM, M_REGDATE FROM MEMBERS WHERE M_ID=#{mId}
	
	-- 이메일 하나 출력(SELECT ONE)
	...
	-- 내찜 조회하기
	SELECT DISTINCT p.P_NUM, p.P_PRICE, p.P_NAME, p.P_AMOUNT, p.P_IMAGE1 FROM PRODUCT p, MYLIKE l, MEMBERS m WHERE p.P_NUM = l.ML_PID AND l.ML_MID = #{mlMid}
	
	-- 내펀딩 조회하기
	SELECT DISTINCT p.P_NUM, p.P_PRICE, p.P_NAME, p.P_AMOUNT, p.P_IMAGE1 FROM PRODUCT p, MYFUNDING f, MEMBERS m WHERE p.P_NUM = f.MF_PID AND f.MF_MID = #{mfMid}
...
-- SAMPLE DATA --
	...
	INSERT INTO PRODUCT( P_NUM, P_NAME, P_PRICE, P_IMAGE1, P_IMAGE2, P_IMAGE3, P_AMOUNT) VALUES( (SELECT NVL(MAX(P_NUM), 0) + 1 FROM PRODUCT), '목성', '5000', 'DEFAULT.jpg', 'DEFAULT.jpg', 'DEFAULT.jpg', '20000');
	...
	INSERT INTO MEMBERS(M_ID, M_PW, M_NAME, M_EMAIL1, M_EMAIL2, M_NUM, M_REGDATE) VALUES('lee','1234','이재환','leedb','naver.com','010-1234-5678',to_char(sysdate,'rr.mm.dd'));

	-- LISTS
	SELECT COUNT(L_NUM) FROM LISTS WHERE L_PID=101;

	INSERT INTO LISTS(L_NUM, L_PID, L_MID) VALUES( '101', '101', 'tester');

	INSERT INTO LISTS(L_NUM, L_PID, L_MID) VALUES( (SELECT NVL(MAX(L_NUM), 0) + 1 FROM LISTS), '101', 'earthman');
