...
//         System.out.println("로그 kakao 확인 id:"+vo.getmId()+"/mName:"+vo.getmName()+"/mEmail1:"+vo.getmEmail1());
   ...
   // 찜목록
      ...
      // 찜목록 -  삭제
            @RequestMapping(value = "/shopcartDelete.do", method = RequestMethod.POST)
            @ResponseBody
            public String deleteLikeList(@RequestBody HashMap<String,List> objParams, Model model, MyLikeVO mlvo, HttpSession session) {

               System.out.println("들어옴"+objParams );
               System.out.print(objParams.getClass().getName());
               MemberVO mvo = (MemberVO) session.getAttribute("member");
