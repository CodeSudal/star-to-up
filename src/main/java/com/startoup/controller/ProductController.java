...
public class ProductController {
	...
	public String selectOne(ProductVO pvo, MyFundingVO myvo, MemberVO mvo, Model model, String msg, ListVO lvo, HttpSession session) {
		mvo = (MemberVO) session.getAttribute("member");
		myvo.setMfMid(mvo.getmId());
		myvo.setMfPid(pvo.getpNum());
		
		// 이 사람이 이 상품을 구매한적이있니?
		if (memberSI.checkDuplication(myvo) != null) { // 이미 구매한 적 있으면
			msg = "checkDuplication";
			model.addAttribute("msg", msg);
			return "alert.jsp";
		} // 없으면 결제 가능
		model.addAttribute("product", productSI.selectOne(pvo));
		model.addAttribute("member", memberSI.selectOneMember(mvo));
		model.addAttribute("list", listSI.sellCount(lvo));
		return "payment.jsp";
	}
